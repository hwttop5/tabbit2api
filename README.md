...
1. 启动时，程序会从本机 Tabbit 用户目录复制出一个独立运行 profile 到仓库内 `.lab/tabbit-user-data`。
2. 通过 Playwright 以持久化浏览器上下文启动本机 `Tabbit.exe`。
...
当前代码默认面向 Windows 环境，且按下面的默认路径寻找 Tabbit：

```text
%USERPROFILE%\AppData\Local\Tabbit\Application\Tabbit.exe
```

同时默认从这里复制用户数据：

```text
%USERPROFILE%\AppData\Local\Tabbit\User Data
```
...
1. 运行 `npm run login`
2. 在弹出的 Tabbit 窗口里完成登录
3. 登录成功后，回到终端按 `Ctrl+C`
4. 再运行 `npm start`
5. 将客户端的 Base URL 指向 `http://127.0.0.1:50124/v1`
...
```text
<repo>/.lab/openai-assistants-state.json
```
...
- [hermes-home/config.yaml](C:\Users\ttop5\Documents\projects\tabbit2api\hermes-home\config.yaml)
...
- `TABBIT_EXECUTABLE`
  - 默认值：`%USERPROFILE%\AppData\Local\Tabbit\Application\Tabbit.exe`
- `TABBIT_USER_DATA_DIR`
  - 默认值：`%USERPROFILE%\AppData\Local\Tabbit\User Data`

### 运行 profile 与调试输出

- `TABBIT_LAB_ROOT`
  - 默认值：`<repo>/.lab`
  - 实际运行 profile 位于 `<repo>/.lab/tabbit-user-data`
- `TABBIT_ASSISTANTS_STATE_PATH`
  - 默认值：`<repo>/.lab/openai-assistants-state.json`
...
程序不会直接操作你正在使用的主 Tabbit 用户目录，而是复制一份较轻量的运行 profile 到仓库内。
...
本地提交会通过 `husky` 和 `commitlint` 做校验。更多约定见 [CONTRIBUTING.md](C:\Users\ttop5\Documents\projects\tabbit2api\CONTRIBUTING.md)。

## 许可证

[GPL-3.0-only](C:\Users\ttop5\Documents\projects\tabbit2api\LICENSE)
